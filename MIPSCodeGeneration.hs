...
convertToMIPS stringTable (MOVE t1 t2) = "move $" ++ t1 ++ ", $" ++ t2
convertToMIPS stringTable (MOVEI t n) = "li $" ++ t ++ ", " ++ show n
...
convertToMIPS stringTable (MOD t1 t2 t3) = 
    "rem $" ++ t1 ++ ", $" ++ t2 ++ ", $" ++ t3
convertToMIPS stringTable (LABEL label) = label ++ ":"
convertToMIPS stringTable (JUMP label) = "j " ++ label
